...
_web3EthContract2.default.setProvider(_config2.default.rpcEndpoint);
const contract = new _web3EthContract2.default(_Assets2.default.abi, _config2.default.contractAddress);

const createAsset = (uuid, payload, address, callback) => {
    return Promise.resolve().then(() => {
        if (_config2.default.storageMode == _config2.default.storageType.IPFS) {
            return (0, _ipfs.uploadToIPFS)(payload);
        } else if (_config2.default.storageMode == _config2.default.storageType.IOTA) {
        ...
        return contract.methods.createAsset(uuidToByte32, value).send({
            from: address,
            gas: _config2.default.gasLimit
        });
    }).then(res => {
        callback({ TxRecipt: res, StorageType: _config2.default.storageMode });
    }).catch(err => {
        callback(err);
    });
};
